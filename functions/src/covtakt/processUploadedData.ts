...
async function sendPushNotificationToUsers(users: UserInfectedInfo[]){
  ...
  await Promise.all(
    users.filter(row => row.isInfected).map(async userInfo => {
      ...
      const notification = {
        title: 'Covtakt',
        body: `You had a contact with COVID-19, ${formatTimestamp(userInfo.timestamp, 'DD.MM.YYYY. HH:mm')}, at a distance of less then ${userInfo.distance} m for ${Math.round(userInfo.duration)} minutes`
      }
